...
class FrontEnd:
    def frontEnd():
        ...
        ticker = ticker.upper()     
        #if ticker.islower():
        #   print("Ticker symbol must be all capital letters. Please re-run the program.") 
        #    exit()
        print("The length of the string (ticker symbol) is", end = ' ')
        print(len(ticker))
        if len(ticker) > 7:
            print("Please enter correct ticker symbol and length (Max length is 7 characters). Please re-run the program.") 
            exit()
